export const JOB_CATEGORIES = [
  "Agency For Barani Area Development",
  "Agriculture & Livestock",
  "Agriculture Department",
  "Allied Hospital, Faisalabad",
  "Armed Forces (Army, Navy, PAF)",
  "Arts & Graphic Design",
  "Aviation Flight (RW), S&GAD",
  "Ayub Agricultural Research Institute, Faisalabad",
  "Aziz Bhatti Shaheed Teaching Hospital, Gujrat",
  "BAHAWALPUR WASTE MANAGEMENT COMPANY",
  "Banking & Finance",
  "Bhakkar Job Bureau",
  "Board of Management Sundar Industrial Estate",
  "Board of Revenue Settlement & Rehabilition Wing",
  "Board of Revenue, Punjab",
  "Bureau of Statistics, Govt. of Punjab",
  "COMMUNICATION AND WORKS DEPARTMENT, GOVERNMENT OF THE PUNJAB",
  "Center Of Excellence Govt  Boys",
  "Ch. Pervaiz Elahi Institute of Cardiology, Multan",
  "Children Library Complex",
  "Children's Hospital & Institute of Child Health Multan",
  "Civil Hospital Bahawalpur",
  "Civil Service (CSS/PMS)",
  "Commissioner Office Rawalpindi",
  "Conservator of Forests Development & Working Plan Circle Lahore, FW&F Department",
  "Content Writing",
  "D.H.Q/ Teaching Hospital",
  "DCO Office Bhakkar",
  "DCO Office Hafizabad",
  "DCO Office Lodhran",
  "DCO Office Multan",
  "DG Khan Medical Collage",
  "DHQ Hospital Faislabad",
  "DHQ/Teaching Hospital Gujranwala",
  "Daanish Schools and Centers of Excellence Authority",
  "Department of Fisheries Punjab, Project Director Aquaculture Mall PIU Lahore",
  "Department of Tourist Services (DTS), Punjab",
  "Deputy Commissioner Office  Katchehry Road  Bhakkar",
  "Deputy Director Fisheries (Pangasius Breeding), Fish Hatchery, Bhaseen, Lahore",
  "Dera Ghazi Khan Waste Management Company",
  "Directorate Civil Defence Punjab",
  "Directorate General Industries, Prices, Weights and Measures",
  "Directorate General Monitoring and Evaluation, Planning and Development Board",
  "Directorate General Public Relation (DGPR)",
  "Directorate General Public Relations (Digital Media Wing)",
  "Directorate General of Archaeology, Government of the Punjab",
  "Directorate General of Labour Welfare, Punjab",
  "Directorate General, Soil Survey of Punjab, Lahore",
  "Directorate of Consumer Protection Council",
  "Directorate of Rapid Soil Fertility Survey & Soil Testing Institute Punjab, Lahore.",
  "District Council",
  "District Council  Sahwial",
  "District Council Multan",
  "District Education Authority Chakwal",
  "District council Gujrat",
  "Education (Schools & Universities)",
  "Engineering (Civil, Tech, etc.)",
  "Engineering Consultancy",
  "Environment Protection",
  "Excise and Taxation",
  "Faisalabad Institute of Cardiology,Faisalabad",
  "Faisalabad Medical University, Faisalabad",
  "Faisalabad Waste Management Company",
  "Fatima Jinnah Institute of Dental Sciences",
  "Fatima Jinnah Medical University",
  "Forest Secretariat",
  "Government (Federal)",
  "Government (Provincial)",
  "Government College Women University Faisalabad",
  "Government General Hospital, Ghulam Muhammad Abad, Faisalabad",
  "Government Mozang Teaching Hospital",
  "Governor's Secretariat",
  "Govt. Kot Khawaja Saeed Teaching Hospital",
  "Govt. Mian Munshi DHQ Teaching Hospital Lahore",
  "Govt.Fatima Jinnah College (W) Chuna Mandi Lahore",
  "Gujranwala Waste Management Company",
  "Gujrat Waste Management Company",
  "HUD&PHED",
  "Healthcare & Medical",
  "Higher Education Commission (HEC)",
  "Holy Family Hospital Rawalpindi.",
  "Home Department",
  "Home Department Punjab",
  "Home Department, Punjab",
  "Hospitality & Tourism",
  "Housing, Urban Development & Public Health Engineering Department",
  "Human Resources (HR)",
  "Human Rights and Minorities Affairs Department",
  "Hydraulic Structures Safety Evaluation Unit (HSSEU)",
  "IIM PU",
  "IT & Software Development",
  "IT Based State Land Management System",
  "Industries, Commerce & Investment Department",
  "Information & Culture",
  "Information and Culture Department",
  "Infrastructure Development Authority of the Punjab",
  "Judiciary / Law",
  "King Edward Medical University",
  "LYALLPUR MUSEUM FAISALABAD",
  "Labor & Human Resource",
  "Labour Department",
  "Lahore General Hospital, Lahore",
  "Lahore Knowledge Park Company",
  "Lahore Museum",
  "Lahore Parking Company",
  "Lahore Waste Management Company",
  "Law and Parliamentary Affairs",
  "Literacy & Non Formal Basic Education",
  "Livestock and Dairy Development",
  "Local Government & Community Development",
  "Logistics & Supply Chain",
  "M C Gujrat",
  "M&R Executive Engineer Division Faislabad",
  "M.C. Isa Khel",
  "MC Gujranwala",
  "MC Kamoke",
  "MC Pasrur",
  "MC Rajanpur",
  "Management & Admin",
  "Management & Professional Development Department",
  "Management and Professional Development",
  "Media & Journalism",
  "Metropolitan Corporation Lahore",
  "Mines & Minerals",
  "Multan Waste Management Company, Multan",
  "Municipal Committee Kot Abdul Malik",
  "Municipal Committee Kot Chutta",
  "Municipal Committee Layyah",
  "Municipal Committee Nowshera Virkan",
  "Municipal Corporation Faisalabad",
  "MunicipalCommittee Pindi Gheb",
  "NGOs & Social Services",
  "National Skills University",
  "Nishtar Medical University",
  "Office of Chief Engineer (Power), Energy Department",
  "Office of Reconciliation Cell, Energy Department, Govt. of the Punjab",
  "Office of the OMBUDSMAN Punjab",
  "Office of the chief architect Communication and Works Department, Punjab",
  "Ombudsperson Punjab",
  "Other",
  "P&D,Program Implementation Unit",
  "PMU Establishment Masstransit System &#40;Gujranwala and Faisalabad&#41;",
  "PMU Punjab Development Programe, LG&CD Department",
  "PMU,PICIIP and PRF LG & CD department",
  "PUNJAB AGRICULTURE COMMISSION",
  "PUNJAB PUBLIC HEALTH AGENCY (PPHA)",
  "Pakistan Kidney & Liver Institute and Research Center (PKLI & RC)",
  "Pakistan Revenue Automation Limited (PRAL)",
  "Pest Warning & Quality Control of Pesticides, Punjab",
  "Planning & Development Department",
  "Planning & Evaluation Cell, Agriculture Department",
  "Police & Law Enforcement",
  "Population Welfare Department",
  "Postgraduate Medical Institute / Ameer ud Din Medical College",
  "Primary & Secondary Healthcare Department",
  "Private Sector",
  "Program Coordination Unit of Punjab Green Development Program",
  "Program Management Unit (PMU), Energy Department",
  "Program Support Unit (PSU), PRIDE Program, Planning & Development Board",
  "Project Director Up Scaling of Green Pakistan Programme",
  "Project Director, Chief Minister Plant for Pakistan",
  "Project Implementation Unit Improving Workforce Readiness in Punjab project",
  "Project Management Unit (PMU), Primary & Secondary Healthcare Department",
  "Project Management Unit (PMU), Punjab Arterial  Roads Improvement Programme (PARIP), C&W  Department, Lahore.",
  "Project Management Unit (PMU), sports Board Punjab",
  "Project Management Unit Shrimp Aquaculture",
  "Public Private Partnership Cell, P&D Department Govt. of Punjab",
  "Public Prosecution Department",
  "Punajb Forestry, Wildlife and Fisheries Department",
  "Punjab Agriculture And Meat Company",
  "Punjab Agriculture(Extension Service)",
  "Punjab Agriculture, Food & Drug Authority (PAFDA)",
  "Punjab Bio Energy Company",
  "Punjab Board of Investment and Trade (PBIT)",
  "Punjab Cattle Market Management & Development Company",
  "Punjab Clean Air Program",
  "Punjab Cooperative Departments",
  "Punjab Curriculum and Textbook Board",
  "Punjab Disaster Management Authority , Board of Revenue",
  "Punjab Economic Research Institute (PERI)",
  "Punjab Education Foundation",
  "Punjab Education Foundation (PEF)",
  "Punjab Education Initiatives Management Authority",
  "Punjab Education, Curriculum, Training and Assessment Authority (PECTAA)",
  "Punjab Educational Endowment Fund (PEEF)",
  "Punjab Energy Deparment",
  "Punjab Enforcement & Regulatory Authority",
  "Punjab Environment Protection Department",
  "Punjab Finance Department",
  "Punjab Food Authority",
  "Punjab Forensic Science Agency",
  "Punjab Forest Department",
  "Punjab Forest Department (GIS Lab)",
  "Punjab Forest School Bahawalpur",
  "Punjab Grid Company Limited",
  "Punjab Halal Development Agency (PHDA)",
  "Punjab Health Facilities Management Company",
  "Punjab Health Foundation (PHF)",
  "Punjab Health Initiatives Management Company",
  "Punjab Higher Education Department",
  "Punjab Home Department",
  "Punjab Housing & Town Planning Agency (PHATA)",
  "Punjab Human Capital Investment Project (PHCIP)",
  "Punjab Human organ Transplant Authority, Lahore.",
  "Punjab Industrial Estates Development And Management Company",
  "Punjab Information Technology Board",
  "Punjab Institute of Language, Art & Culture",
  "Punjab Irrigation Department",
  "Punjab Land Record Authority",
  "Punjab Land Records Authority",
  "Punjab Livestock & Dairy Development Board",
  "Punjab Mineral Company",
  "Punjab Model Bazaars Management Company",
  "Punjab Municipal Development Fund Company (PMDFC)",
  "Punjab Pension Fund",
  "Punjab Population Innovation Fund",
  "Punjab Power Development Company Limited  (PPDCL)",
  "Punjab Power Management Unit, Energy Department",
  "Punjab Prisons",
  "Punjab Provincial Transport Authority",
  "Punjab Public Private Partnership Authority",
  "Punjab Resource Management & Policy Unit",
  "Punjab Resource Management Program",
  "Punjab Road Safety Authority",
  "Punjab Rural Municipal Services Company",
  "Punjab Saaf Pani Authority",
  "Punjab Saaf Pani Company - South",
  "Punjab Seed Corporation",
  "Punjab Skills Development Authority",
  "Punjab Skills Development Fund",
  "Punjab Social Protection Authority",
  "Punjab Social Security Health Management Company",
  "Punjab Sports Board",
  "Punjab TEVTA",
  "Punjab Thalassemia and Other Genetic Disorders Prevention and Research Institute (PTGD), Fatima Jinnah Medical University, Lahore",
  "Punjab Thermal Power Pvt Ltd  Energy Department",
  "Punjab Tianjin University of Technology",
  "Punjab Tourism for Economic Growth project (PTEGP)",
  "Punjab Transport Company (PTC)",
  "Punjab Water and Sanitation Authority",
  "Punjab Wildlife and Parks Department",
  "Punjab commision on status of women (PSCW)",
  "Punjab energy efficiency and conservation agency (PEECA)",
  "QUAIDEAZAM THERMAL POWER (PVT) LIMITED (ENERGY DEPARTMENT)",
  "Quaid e Azam Solar Power Pvt Limited",
  "Rapid Soil Fertility Survey & Soil Testing Institute",
  "Rawalpindi Development Authority",
  "Rawalpindi Medical University",
  "Rawalpindi Waste Management Company",
  "Regulations Wing, S&GAD",
  "S&GAD (I&C Wing)",
  "S.E  Building (M&R) Circle Faisalabad",
  "Sahiwal Water & Sanitation Services Company",
  "Sales & Marketing",
  "Sargodha Waste Management Company",
  "School Education Department",
  "Science & Research",
  "Service Institute of Medical Science",
  "Services & General Administration Department",
  "Services Department",
  "Sir Ganga Ram Hospital, Lahore.",
  "Social Welfare department",
  "South Punjab Forest Company",
  "Special Education Department",
  "Special Judge Anti,Corruption",
  "Special Monitoring Unit Chief Mister's Office (SMU)",
  "Syed Muhamamd Hussain Govt T.B Sanatorium Samli.",
  "The Lahore Chamber of Commerce & Industry",
  "The Punjab Masstransit Authority",
  "The Urban Unit",
  "Tourism, Archaeology and Museums Department",
  "Transport Planning Unit",
  "University of Education",
  "University of Home Economics Lahore",
  "VIP Flight, S&GAD",
  "Veterinary Research Institute",
  "WASA Rawalpindi",
  "WASA, LDA Lahore",
  "Walled City of Lahore Authority",
  "Water & Sanitation Agency (WASA) Lahore",
  "Water and sanitation agency, Faisalabad",
  "Wazirabad Institute of Cardiology",
  "Women Development Department",
  "Youth Affairs, Sports, Archaeology and Tourism Department",
  "Zakat & Ushr Department",
  "center for professional development of public prosecutors",
  "social welfare  baitulMaal Department"
];
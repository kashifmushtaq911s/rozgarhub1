...
export default function AdminJobsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'published': return <span className="bg-blue-500/10 text-blue-600 border border-blue-100 px-3 py-1 rounded-xl text-xs font-black uppercase tracking-widest">Active</span>
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-slate-900 mb-2">Job <span className="text-blue-600">Inventory</span></h1>
          <p className="text-slate-500 font-medium">Manage and monitor live opportunities across Pakistan.</p>
        </div>
        <Link href="/admin/jobs/new">
          <Button className="h-14 px-8 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-black shadow-xl shadow-blue-500/20 gap-3 transition-all hover:scale-[1.02] active:scale-[0.98]">
        ...
        <CardContent className="p-8 flex flex-col md:flex-row gap-6">
          ...
          <div className="flex gap-4">
            <div className="relative min-w-[160px]">
              <select className="appearance-none h-14 w-full rounded-2xl border border-slate-100 bg-slate-50 px-6 text-sm font-black text-slate-600 focus:outline-none focus:ring-4 focus:ring-blue-100 cursor-pointer">
            ...
            <Button variant="outline" className="h-14 px-6 rounded-2xl border-slate-100 bg-slate-50 text-slate-500 font-bold hover:bg-white hover:border-blue-200 hover:text-blue-600 transition-all">
              Advanced Filters
            </Button>
          </div>
        </CardContent>
              ...
              {jobs.length > 0 ? jobs.map((job) => (
                <tr key={job.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-all group">
                  ...
                  <td className="px-6 py-5 text-center">
                     <span className="font-black text-blue-500 bg-blue-50 px-2 py-1 rounded-lg text-[10px]">{job.scale || "N/A"}</span>
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                       <Link href={`/job/${(job.title || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}`} target="_blank">
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg text-slate-300 hover:text-blue-600 hover:bg-blue-50 transition-all"><Eye size={16} /></Button>
                       </Link>
                ...
                <tr>
                  <td colSpan={6} className="p-24 text-center">
                    <div className="flex flex-col items-center justify-center space-y-6">
                      ...
                      <Link href="/admin/jobs/new">
                        <Button className="h-12 px-8 rounded-xl bg-white border-2 border-blue-600 text-blue-600 font-black hover:bg-blue-600 hover:text-white transition-all shadow-lg shadow-blue-500/10">Publish Early Access Job</Button>
                      </Link>
                    </div>
                  </td>
                </tr>
        ...
        {jobs.length > 0 && (
          <div className="p-8 bg-slate-50/30 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-3">
              ...
              <Button variant="outline" size="sm" className="h-10 px-6 rounded-xl border-blue-200 text-blue-600 font-bold" disabled>Next Level</Button>
            </div>
          </div>
        )}
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
         <div className="bg-blue-600 rounded-[3rem] p-12 text-white shadow-2xl shadow-blue-500/30 flex justify-between items-center relative overflow-hidden group">
            <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-white/10 blur-3xl rounded-full"></div>
            <div className="space-y-2 relative z-10">
               <p className="text-blue-200 font-black text-[10px] uppercase tracking-[0.2em]">Total Postings</p>
               <h3 className="text-4xl font-black">{jobs.length} Jobs</h3>
               <p className="text-blue-100/60 font-medium text-sm">Total listings in the database.</p>
            </div>
            <div className="h-16 w-16 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-md relative z-10">
               <TrendingUp size={32} />
            </div>
         </div>
         <div className="bg-blue-500 rounded-[3rem] p-12 text-white shadow-2xl shadow-blue-500/30 flex justify-between items-center relative overflow-hidden group">
             <div className="absolute -bottom-10 -right-10 w-48 h-48 bg-white/10 blur-3xl rounded-full"></div>
            <div className="space-y-2 relative z-10">
               <p className="text-blue-100 font-black text-[10px] uppercase tracking-[0.2em]">Conversion Rate</p>
               <h3 className="text-4xl font-black">100% Secure</h3>
               <p className="text-blue-50/60 font-medium text-sm">All listings verified and active.</p>
            </div>
            <div className="h-16 w-16 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-md relative z-10">
               <Sparkles size={32} />
            </div>
